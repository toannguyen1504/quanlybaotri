...
public class DashboardController {
    ...
    public DashboardService.Dashboard dashboard(@RequestParam(required = false) Instant from,
            @RequestParam(required = false) Instant to) {
        Instant end = to == null ? Instant.now() : to;
        Instant start = from == null ? end.minus(30, java.time.temporal.ChronoUnit.DAYS) : from;
