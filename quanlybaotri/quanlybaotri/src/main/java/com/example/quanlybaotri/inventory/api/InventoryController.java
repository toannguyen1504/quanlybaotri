...
public class InventoryController {
    ...
    public record TicketPartView(UUID id, UUID partId, String partCode, String partName, BigDecimal quantity,
        ...
        public static TicketPartView from(TicketPart p) {
            return new TicketPartView(p.getId(), p.getPart().getId(), p.getPart().getCode(), p.getPart().getName(),
                    p.getQuantity(), p.getUnitCost(), p.getUsedBy().getId(), p.getUsedBy().getFullName(),
                    p.getUsedAt());
        }
    }
    ...
    public record MovementView(UUID id, UUID partId, String partCode, UUID ticketId, String ticketCode,
        ...
        public static MovementView from(StockMovement m) {
            return new MovementView(m.getId(), m.getPart().getId(), m.getPart().getCode(),
                    m.getTicket() == null ? null : m.getTicket().getId(),
                    m.getTicket() == null ? null : m.getTicket().getCode(), m.getType(), m.getQuantity(),
                    m.getBalanceAfter(), m.getUnitCost(), m.getReason(), m.getActor().getFullName(), m.getCreatedAt());
        }
    }
}
