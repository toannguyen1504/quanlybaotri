...
public class InternalEquipmentController {
    ...
    public InternalEquipmentController(EquipmentRepository repo) {
        this.repo = repo;
    }

    @GetMapping("/{id}")
    @Transactional(readOnly = true)
    public EquipmentRef get(@PathVariable UUID id) {
        return EquipmentRef.from(
            repo.findById(id).orElseThrow(() -> ApiException.notFound("Không tìm thấy thiết bị"))
        );
    }

    @PostMapping("/resolve")
    @Transactional(readOnly = true)
    public List<EquipmentRef> resolve(@Valid @RequestBody ResolveRequest r) {
        return repo
    ...
    public record EquipmentRef(
        ...
        static EquipmentRef from(Equipment e) {
