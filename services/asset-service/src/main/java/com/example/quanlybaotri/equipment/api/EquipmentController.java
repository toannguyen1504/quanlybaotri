...
public class EquipmentController {
    ...
    public Page<View> list(
        ...
        var names = organizations.resolve(
            result
                .getContent()
                .stream()
        ...
        return result.map(e ->
            View.from(e, e.getDepartmentId() == null ? null : names.get(e.getDepartmentId()))
        );
    }
    ...
    public View get(@PathVariable UUID id) {
        ...
        var department =
            e.getDepartmentId() == null ? null : organizations.find(e.getDepartmentId());
    ...
    public View create(@Valid @RequestBody Request r) {
        ...
        Equipment e = transactions.execute(tx -> {
            if (repo.existsByAssetCodeIgnoreCase(r.assetCode())) throw ApiException.conflict(
                "Mã thiết bị đã tồn tại"
            );
            Equipment created = new Equipment(
                r.assetCode().trim().toUpperCase(),
                r.name().trim(),
                category(r.categoryId())
            );
            apply(created, r, department);
    ...
    public View update(@PathVariable UUID id, @Valid @RequestBody Request r) {
        var department = department(r.departmentId());
        Equipment e = transactions.execute(tx -> {
            Equipment found = find(id);
            apply(found, r, department);
