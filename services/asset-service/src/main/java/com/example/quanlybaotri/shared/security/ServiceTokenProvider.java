...
public class ServiceTokenProvider {
    ...
    public synchronized String token(String audience) {
        ...
        TokenResponse r = client
            .post()
            .uri(u -> u.path("/internal/v1/auth/token").queryParam("audience", audience).build())
            .headers(h -> h.setBasicAuth("asset-service", secret))
            .retrieve()
            .body(TokenResponse.class);
        if (r == null) throw new IllegalStateException("No service token");
