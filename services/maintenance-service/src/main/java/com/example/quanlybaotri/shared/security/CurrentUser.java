...
public class CurrentUser {
    public Actor require() {
        ...
        if (!"user".equals(tokenType) || uid == null) throw ApiException.forbidden("Yêu cầu token người dùng");
        Set<RoleName> roles = auth.getToken().getClaimAsStringList("roles").stream()
                .map(value -> value.replaceFirst("^ROLE_", ""))
                .map(RoleName::valueOf).collect(Collectors.toUnmodifiableSet());
        return new Actor(UUID.fromString(uid), auth.getName(), auth.getToken().getClaimAsString("name"), roles,
                auth.getToken().getTokenValue());
    }

    public record Actor(UUID id, String username, String fullName, Set<RoleName> roles, String token) {
        public boolean has(RoleName role) { return roles.contains(role); }
    }
}
