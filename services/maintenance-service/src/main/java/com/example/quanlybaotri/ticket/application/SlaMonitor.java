...
public class SlaMonitor {
    ...
    public void scan() {
        ...
        for (MaintenanceTicket t : tickets.findByStatusNotIn(TERMINAL)) {
            if (t.getAcceptedAt() == null) check(t, "SLA_RESPONSE", t.getResponseDueAt(), now);
            if (t.getResolvedAt() == null) check(t, "SLA_RESOLUTION", t.getResolutionDueAt(), now);
        }
    }
    ...
    private void check(MaintenanceTicket t, String prefix, Instant due, Instant now) {
        ...
        if (!now.isBefore(due)) {
            ...
            if (!events.existsByTicketIdAndEventType(t.getId(), type)) publisher.record(
                t,
                null,
                type,
                t.getStatus(),
                t.getStatus(),
                "Phiếu đã quá hạn SLA",
            ...
            if (!events.existsByTicketIdAndEventType(t.getId(), type)) publisher.record(
                ...
                t.getStatus(),
                t.getStatus(),
                "Phiếu đã sử dụng 80% thời gian SLA",
                Map.of("dueAt", due.toString())
            );
        }
    }
