...
public class SlaPolicyController {
    ...
    public View update(@PathVariable UUID id, @Valid @RequestBody Request request) {
        SlaPolicy policy = repo.findById(id).orElseThrow(() -> ApiException.notFound("Không tìm thấy chính sách SLA"));
        policy.update(request.responseMinutes(), request.resolutionMinutes(), request.active());
        return View.from(policy);
    }

    public record Request(@Min(1) int responseMinutes, @Min(1) int resolutionMinutes, boolean active) {
    }

    public record View(UUID id, TicketPriority priority, int responseMinutes, int resolutionMinutes, boolean active,
            long version) {
        static View from(SlaPolicy p) {
            return new View(p.getId(), p.getPriority(), p.getResponseMinutes(), p.getResolutionMinutes(), p.isActive(),
                    p.getVersion());
        }
    }
}
