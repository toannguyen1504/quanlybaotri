...
public class ServiceTokenProvider {
    ...
    public synchronized String token(String audience) {
        ...
        if (c != null && c.expiresAt.isAfter(now.plusSeconds(30))) return c.value;
        TokenResponse r = client
            ...
            .headers(h -> h.setBasicAuth("inventory-service", secret))
            .retrieve()
            .body(TokenResponse.class);
        if (r == null) throw new IllegalStateException("Identity returned no service token");
        cache.put(audience, new Cached(r.accessToken(), now.plusSeconds(r.expiresIn())));
